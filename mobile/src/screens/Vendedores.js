...
export default function Vendedores() {
  ...
  function Avatar({ foto, nombre, size = 52 }) {
    ...
    if (foto) return <Image source={{ uri: foto }} style={{ width: size, height: size, borderRadius: size / 2, borderWidth: 2, borderColor: C.moradoClaro }} />;
    return (
      <View style={{ width: size, height: size, borderRadius: size / 2, backgroundColor: C.morado, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: C.moradoClaro }}>
    ...
    return (
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 14, marginVertical: 8 }}>
        <TouchableOpacity onPress={pickImage} style={{ position: 'relative' }}>
          ...
          <View style={{ position: 'absolute', bottom: 0, right: 0, backgroundColor: C.morado, borderRadius: 11, width: 22, height: 22, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: C.surface }}>
            <Text style={{ fontSize: 10 }}>📷</Text>
          </View>
        </TouchableOpacity>
        <View style={{ flex: 1 }}>
          <TouchableOpacity onPress={pickImage}>
            <Text style={{ color: C.morado, fontWeight: '700', fontSize: 13 }}>{value ? 'Cambiar foto' : 'Agregar foto'}</Text>
          </TouchableOpacity>
          {value && <TouchableOpacity onPress={() => onChange(null)}><Text style={{ color: C.rojo, fontSize: 12, marginTop: 4 }}>Quitar foto</Text></TouchableOpacity>}
  ...
  async function handleCreate() {
    ...
    try {
      ...
      showToast('Vendedor creado'); setForm({ nombre: '', foto: null }); setShowForm(false); loadVendedores();
  ...
  async function handleUpdate(id) {
    ...
    try {
      ...
      showToast('Vendedor actualizado'); setEditId(null); loadVendedores();
    } catch (err) { showToast(err.message, 'error'); }
  }

  async function handleDelete(id) {
    try {
      await vendedoresService.delete(id); showToast('Vendedor eliminado'); setConfirmId(null); loadVendedores();
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      ...
      {confirmId && <Confirm message="¿Eliminar este vendedor? Solo es posible si no tiene ventas." onConfirm={() => handleDelete(confirmId)} onCancel={() => setConfirmId(null)} />}
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 24 }}>
        ...
        {showForm && (
          <View style={[s.card, { borderColor: C.rosa, borderWidth: 2 }]}>
            ...
            <TextInput style={s.input} placeholder="Ej: María López" placeholderTextColor={C.textMuted} value={form.nombre} onChangeText={v => setForm(f => ({ ...f, nombre: v }))} />
            <View style={{ flexDirection: 'row', gap: 10, marginTop: 16 }}>
              <TouchableOpacity style={[s.btn, s.btnGhost]} onPress={() => { setShowForm(false); setForm({ nombre: '', foto: null }); }}>
            ...
            <View key={v.id} style={[s.card, i < 3 && { borderColor: rankColors[i] + '66', borderWidth: 2 }]}>
              {editId === v.id ? (
                <View>
                  ...
                  <TextInput style={[s.input, { borderColor: C.rosa, borderWidth: 2, marginBottom: 12 }]} value={editForm.nombre} onChangeText={nombre => setEditForm(f => ({ ...f, nombre }))} />
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                  ...
                  <View style={{ flexDirection: 'row', gap: 8 }}>
                    <TouchableOpacity style={s.iconBtn} onPress={() => { setEditId(v.id); setEditForm({ nombre: v.nombre, foto: v.foto || null }); }}>
                      <Text>✏️</Text>
                    </TouchableOpacity>
