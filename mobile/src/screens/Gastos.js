...
export default function Gastos() {
  ...
  async function handleCrearCategoria() {
    ...
    try {
      ...
      setNuevaCat(''); setShowCatForm(false);
      showToast('Categoría creada'); loadCategorias();
  ...
  async function handleEliminarCategoria() {
    try {
      ...
      setConfirmCat(null); showToast('Categoría eliminada');
      loadCategorias(); loadGastos();
    } catch { setConfirmCat(null); showToast('No se pudo eliminar', 'error'); }
  }

  function handleEdit(g) {
    setForm({ descripcion: g.descripcion, monto: String(g.monto), fecha: g.fecha?.split('T')[0] || hoy, categoria_id: g.categoria_id ? String(g.categoria_id) : '' });
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (editId) { await gastosService.update(editId, payload); showToast('Gasto actualizado'); }
      else { await gastosService.create(payload); showToast('Gasto registrado'); }
      handleCancel(); loadGastos();
    } catch (err) { showToast(err.message, 'error'); }
  ...
  async function handleDelete(id) {
    try {
      await gastosService.delete(id); showToast('Gasto eliminado');
      setConfirmId(null); loadGastos();
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      ...
      {confirmCat && <Confirm message="¿Eliminar esta categoría? Los gastos quedarán sin categoría." onConfirm={handleEliminarCategoria} onCancel={() => setConfirmCat(null)} />}
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 24 }}>
          ...
          <View style={[s.card, { backgroundColor: C.lilaPalido, borderColor: C.moradoClaro }]}>
            ...
            {showCatForm && (
              <View style={{ flexDirection: 'row', gap: 8, marginTop: 10 }}>
                <TextInput style={[s.input, { flex: 1 }]} placeholder="Ej: Empaques..." placeholderTextColor={C.textMuted} value={nuevaCat} onChangeText={setNuevaCat} />
            ...
            {categorias.length === 0 && !showCatForm && <Text style={s.muted}>Crea categorías para organizar tus gastos.</Text>}
          </View>
        )}
        ...
        {!showForm && (
          <View style={[s.card, { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }]}>
            ...
            <Text style={{ fontWeight: '800', fontSize: 15, color: C.text, textTransform: 'capitalize' }}>{mesLabel(mes)}</Text>
        ...
        {!showForm && gastos.length > 0 && (
          <View style={[s.card, { backgroundColor: C.lilaPalido, borderColor: C.moradoClaro, borderWidth: 2 }]}>
            ...
            <Text style={{ fontSize: 28, fontWeight: '900', color: C.rojo, marginTop: 4 }}>{fmt(totalMes)}</Text>
          </View>
        )}
        ...
        {showForm && (
          <View style={[s.card, { borderColor: C.lila, borderWidth: 2 }]}>
            ...
            <TextInput style={s.input} placeholder="Ej: Compra de empaques" placeholderTextColor={C.textMuted} value={form.descripcion} onChangeText={v => setForm(f => ({ ...f, descripcion: v }))} />
            <Text style={s.label}>Categoría</Text>
            <PickerModal options={catOptions} value={form.categoria_id} onChange={v => setForm(f => ({ ...f, categoria_id: v ? String(v) : '' }))} placeholder="— Sin categoría —" />
            ...
            <View style={{ flexDirection: 'row', gap: 10, marginTop: 16 }}>
              ...
              <TouchableOpacity style={[s.btn, s.btnPrimary, { flex: 1 }]} onPress={handleSubmit} disabled={saving}>
                {saving ? <ActivityIndicator color="#fff" /> : <Text style={s.btnPrimaryText}>{editId ? 'Actualizar' : 'Guardar'}</Text>}
              </TouchableOpacity>
            </View>
          </View>
        )}
            ...
            Object.entries(porCategoria).map(([catNombre, items]) => (
              <View key={catNombre} style={{ marginBottom: 16 }}>
                <View style={s.catHeader}>
                  ...
                  <Text style={[s.catLabel, { color: C.rojo }]}>{fmt(items.reduce((s, g) => s + Number(g.monto), 0))}</Text>
                </View>
                {items.map(g => (
...
function makeStyles(C) {
  return StyleSheet.create({
    ...
    catBtn: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, borderWidth: 1.5, borderColor: C.morado, backgroundColor: C.surface },
    catBtnText: { color: C.morado, fontWeight: '700', fontSize: 13 },
    chip: { flexDirection: 'row', alignItems: 'center', backgroundColor: C.surface, borderWidth: 1.5, borderColor: C.moradoClaro, borderRadius: 20, paddingHorizontal: 10, paddingVertical: 5 },
    ...
    navBtn: { width: 36, height: 36, borderRadius: 10, backgroundColor: C.lilaPalido, alignItems: 'center', justifyContent: 'center' },
    navBtnText: { fontSize: 20, fontWeight: '700', color: C.morado },
    mesBtn: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, borderWidth: 1.5, borderColor: C.border, backgroundColor: C.bg },
    ...
    catHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6, paddingHorizontal: 4 },
