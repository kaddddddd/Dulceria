...
export default function Reparto() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      ...
      {confirmId && (
        <Confirm message="¿Eliminar este retiro?"
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 24 }}>
        ...
        <View style={[s.card, { backgroundColor: C.lilaPalido, borderColor: C.moradoClaro }]}>
          ...
          <PickerModal
            options={vendOptions}
            value={vendDuena}
            onChange={handleVendDuenaChange}
            placeholder="— Selecciona —"
          />
          {data && !data.tiene_propios && (
            <View style={s.infoBox}>
              <Text style={s.infoText}>
                ℹ️ No hay productos propios. En <Text style={{ fontWeight: '700' }}>Inventario</Text> puedes asignar productos a una persona.
              </Text>
            </View>
          )}
          {data && data.tiene_propios && (
            <View style={s.successBox}>
              <Text style={s.successText}>
                ✅ {data.productos_propios.length} producto{data.productos_propios.length > 1 ? 's propios' : ' propio'} — sus ventas van 100% a su propietaria.
              </Text>
            </View>
          )}
        </View>
        ...
        <View style={s.card}>
          ...
          <View style={{ flexDirection: 'row', gap: 10 }}>
            <View style={{ flex: 1 }}>
              ...
              <PickerModal
                ...
                value={form.persona}
                onChange={v => {
                  const nombre = vendedores.find(vd => String(vd.id) === String(v))?.nombre || '';
                  setForm(f => ({ ...f, persona: nombre }));
                }}
                placeholder="— Selecciona —"
              />
            </View>
          ...
          <View style={{ flexDirection: 'row', gap: 10, marginTop: 4 }}>
            <View style={{ flex: 1 }}>
              ...
              <TextInput style={s.input} placeholder="Ej: adelanto semana"
                value={form.concepto} onChangeText={v => setForm(f => ({ ...f, concepto: v }))} />
            </View>
            <View style={{ flex: 1 }}>
          ...
          {fError ? (
            <View style={s.errorBox}>
              <Text style={s.errorText}>⚠️ {fError}</Text>
            </View>
          ) : null}
        ...
        {error && (
          <View style={s.card}>
            <Text style={{ color: C.rojo, textAlign: 'center' }}>{error}</Text>
          </View>
        )}
            ...
            <>
              ...
              {data.tiene_propios && (
                <View style={[s.card, { borderColor: C.moradoClaro, borderWidth: 2, backgroundColor: C.lilaPalido }]}>
                    ...
                    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
                      {data.productos_propios.map(p => (
                        <View key={p.id} style={s.badge}>
                          <Text style={s.badgeText}>{p.nombre}</Text>
                        </View>
                      ))}
                    </View>
                  )}
                  ...
                  {data.propios.ingresos === 0 && (
                    <Text style={{ fontSize: 13, color: C.textMuted, marginTop: 8 }}>Sin ventas de productos propios en este período.</Text>
                  )}
                </View>
              )}
              ...
              <View style={s.card}>
                ...
                <View style={[s.rowBetween, { marginTop: 8, paddingTop: 8, borderTopWidth: 1, borderTopColor: C.border }]}>
                  ...
                  <Text style={{ fontWeight: '800', fontSize: 16, color: data.general.ganancia >= 0 ? C.verde : C.rojo }}>
              ...
              <View style={{ flexDirection: 'row', gap: 10, marginBottom: 12 }}>
                <View style={[s.statCard, { backgroundColor: '#F0FDF4', borderColor: C.verde }]}>
                  ...
                  <View style={{ borderTopWidth: 1, borderTopColor: C.border, marginTop: 6, paddingTop: 6 }}>
                    <Text style={{ fontSize: 14, fontWeight: '800', color: '#065F46' }}>Pendiente: {fmt(data.distribucion.dueno.pendiente)}</Text>
                  </View>
                </View>
                    ...
                    <View key={r.id} style={[s.rowBetween, { paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: C.border }]}>
                      <View>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                          <View style={s.badge}>
                            <Text style={s.badgeText}>{r.persona}</Text>
                          </View>
                      ...
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                        ...
                        <TouchableOpacity style={s.iconBtn} onPress={() => setConfirmId(r.id)}>
                          <Text>🗑️</Text>
                        </TouchableOpacity>
                      </View>
                    </View>
...
const s = StyleSheet.create({
  ...
  mesBtn: {
    ...
    borderWidth: 1.5, borderColor: C.border, backgroundColor: C.bg, alignSelf: 'flex-start',
  },
  mesBtnText: { fontSize: 13, fontWeight: '700', color: C.text },
  rowBetween: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  statCard: {
    flex: 1, borderRadius: 16, padding: 14, borderWidth: 1.5, gap: 4,
  },
  ...
  successBox: { marginTop: 10, backgroundColor: '#F0FDF4', borderWidth: 1, borderColor: C.verde, borderRadius: 8, padding: 10 },
